...
public HttpResponseData GetStudentsBySchool(
    ...
    string school)
{
    var students = _context.Students.Where(s => s.School == school).ToList();
    var response = req.CreateResponse(HttpStatusCode.OK);
    response.WriteAsJsonAsync(students);
    return response;
}
...
{
    var studentCounts = _context.Students
        .GroupBy(s => s.School)
        .Select(g => new { School = g.Key, Count = g.Count() })
        .OrderByDescending(g => g.Count)
        .ToList();
    var response = req.CreateResponse(HttpStatusCode.OK);
    response.WriteAsJsonAsync(studentCounts);
    return response;
}
